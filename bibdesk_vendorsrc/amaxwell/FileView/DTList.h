...
protected:
    DTList(int len) : Data(len<=0 ? 0 : new T[len]), length(len<=0 ? 0 : len), refCount(new int(1)), outOfRange() {}
public:
    ...
    int MemoryUsed(void) const {return length*sizeof(T);}

    const T *Pointer(void) const {return Data;}

    int IsEmpty(void) const {return (Data==0);}
    int Length(void) const {return length;}

#if DTRangeCheck
    T operator()(int i) const  {if (i<0 || i>= length) {DTErrorOutOfRange("DTList<T>",i,length); return outOfRange;} return Data[i];}
#else
...
public:
    ...
    DTMutableList(int len) : DTList<T>(len) {}
...
#if DTRangeCheck
    T &operator()(int i) {if (i<0 || i>= DTList<T>::length) {DTErrorOutOfRange("DTList<T>",i,DTList<T>::length); return DTList<T>::outOfRange;} return DTList<T>::Data[i];}
    T operator()(int i) const  {if (i<0 || i>= DTList<T>::length) {DTErrorOutOfRange("DTList<T>",i,DTList<T>::length); return DTList<T>::outOfRange;} return DTList<T>::Data[i];}
#else
    T &operator()(int i) {return DTList<T>::Data[i];}
    T operator()(int i) const  {return DTList<T>::Data[i];}
#endif

    DTMutableList<T> &operator=(T v) {for (int i=0;i<DTList<T>::length;i++) DTList<T>::Data[i] = v; return *this;}
};

template <class T> DTMutableList<T> TruncateSize(const DTList<T> &A,int length)
{
    ...
    for (int i=0;i<length;i++) toP[i] = fromP[i];
    return toReturn;
}

template <class T> DTMutableList<T> IncreaseSize(const DTList<T> &A,int addLength)
{
    DTMutableList<T> toReturn(A.Length()+(addLength>=0 ? addLength : 0));
    int len = A.Length();
    const T *fromP = A.Pointer();
    T *toP = toReturn.Pointer();
    for (int i=0;i<len;i++) toP[i] = fromP[i];
    return toReturn;
}
