...
RCS_ID("$Header: svn+ssh://source.omnigroup.com/Source/svn/Omni/tags/OmniSourceRelease_2007-10-25/OmniGroup/Frameworks/OmniAppKit/OpenStepExtensions.subproj/NSDocument-OAExtensions.m 89466 2007-08-01 23:35:13Z kc $");
...
- (void)startingLongOperation:(NSString *)operationName automaticallyEnds:(BOOL)shouldAutomaticallyEnd;
{
    ...
    if ([windowControllers count] != 0) {
        ...
        documentWindow = [controller window];
    } else {
        // Create a tiny 1x1 window in the center of the main screen and display our progress bar over that
        documentWindow = [[NSWindow alloc] initWithContentRect:NSMakeRect(0.0f, 0.0f, 1.0f, 1.0f) styleMask:NSBorderlessWindowMask backing:NSBackingStoreBuffered defer:NO];
        [documentWindow center];
        [documentWindow makeKeyAndOrderFront:nil];
        [documentWindow setReleasedWhenClosed:YES];
        [documentWindow queueSelector:@selector(close)];
    }
	
    [NSWindowController startingLongOperation:operationName controlSize:NSSmallControlSize inWindow:documentWindow automaticallyEnds:shouldAutomaticallyEnd];
}
