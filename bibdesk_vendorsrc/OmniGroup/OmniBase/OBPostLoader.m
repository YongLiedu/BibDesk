...
RCS_ID("$Header: svn+ssh://source.omnigroup.com/Source/svn/Omni/tags/OmniSourceRelease_2007-10-25/OmniGroup/Frameworks/OmniBase/OBPostLoader.m 91600 2007-09-20 18:33:17Z bungi $")
    ...
    while (didInvokeSomething) {
        ...
        if (classes != NULL) {
            ...
            for (classIndex = 0; classIndex < classCount; classIndex++) {
                ...
                if (_objc_resolve_categories_for_class != NULL)
...
+ (BOOL)_processSelector:(SEL) selectorToCall inClass:(Class)aClass initialize:(BOOL)shouldInitialize;
{
    Class metaClass = OB_object_getClass(aClass); // we are looking at class methods
    ...
    //fprintf(stderr, "Checking for implementations of +[%s %s]\n", OB_class_getName(aClass), sel_getName(selectorToCall));
#endif
    ...
    Method *methodList = OB_class_copyMethodList(metaClass, &methodCount);
    for (methodIndex = 0; methodIndex < methodCount; methodIndex++) {
        Method m = methodList[methodIndex];
        
        if (OB_method_getName(m) == selectorToCall) {
            IMP imp = OB_method_getImplementation(m);
...
#if defined(POSTLOADER_DEBUG)
                fprintf(stderr, "Recording +[%s %s] (0x%08x)\n", OB_class_getName(aClass), sel_getName(selectorToCall), (int)imp);
#endif
            }
        }
    }
    ...
    if (impCount) {
        ...
        for (impIndex = 0; impIndex < impCount; impIndex++) {
            ...
            @try {
                ...
                Class imposterClass = objc_getClass(OB_class_getName(metaClass));
                if (imposterClass != Nil)
                    aClass = imposterClass;
                
                imps[impIndex](aClass, selectorToCall);
            } @catch (NSException *exc) {
                fprintf(stderr, "Exception raised by +[%s %s]: %s\n", OB_class_getName(aClass), sel_getName(selectorToCall), [[exc reason] UTF8String]);
            }
#if defined(POSTLOADER_DEBUG)
            fprintf(stderr, "done\n");
#endif
