...
RCS_ID("$Header: svn+ssh://source.omnigroup.com/Source/svn/Omni/tags/OmniSourceRelease_2007-10-25/OmniGroup/Frameworks/OmniFoundation/DataStructures.subproj/OFStaticArray.m 90130 2007-08-15 07:15:53Z bungi $")
...
- (void)removeObjectAtIndex:(unsigned int)objectIndex;
{
    void *targetObjectPointer;
    
    if (objectIndex >= count)
        [NSException raise:NSRangeException format:@"index (%d) is beyond count (%d)", objectIndex, count];

    targetObjectPointer = mutableBytes + objectLength * objectIndex;
    [(id)targetObjectPointer dealloc];
    
    if (objectIndex < count-1) {
        // Fill in the hole with the later objects in the array
        bcopy(targetObjectPointer + objectLength, targetObjectPointer, (count - objectIndex - 1) * objectLength);
    }
...
- (NSMutableDictionary *)debugDictionary;
{
    ...
    for (objectIndex = 0; objectIndex < count; objectIndex++)
        [objects addObject:[self objectAtIndex:objectIndex]];
