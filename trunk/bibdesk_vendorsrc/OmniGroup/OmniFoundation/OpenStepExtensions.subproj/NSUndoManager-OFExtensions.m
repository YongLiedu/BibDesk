...
RCS_ID("$Header: svn+ssh://source.omnigroup.com/Source/svn/Omni/tags/OmniSourceRelease_2007-10-25/OmniGroup/Frameworks/OmniFoundation/OpenStepExtensions.subproj/NSUndoManager-OFExtensions.m 90130 2007-08-15 07:15:53Z bungi $");
...
static void (*original_registerUndoWithTargetSelectorObject)(id self, SEL _cmd, id target, SEL sel, id obj) = NULL;
static void (*original_forwardInvocation)(id self, SEL _cmd, NSInvocation *invocation) = NULL;
static void (*original_undo)(id self, SEL _cmd) = NULL;
static void (*original_redo)(id self, SEL _cmd) = NULL;
static void (*original_beginUndoGrouping)(id self, SEL _cmd) = NULL;
static void (*original_endUndoGrouping)(id self, SEL _cmd) = NULL;
...
#define REPL(old, name) old = (typeof(old))OBReplaceMethodImplementationWithSelector(self, @selector(name), @selector(replacement_ ## name))
            ...
            REPL(original_registerUndoWithTargetSelectorObject, registerUndoWithTarget:selector:object:);
            REPL(original_forwardInvocation, forwardInvocation:);
            REPL(original_undo, undo);
            REPL(original_redo, redo);
            REPL(original_beginUndoGrouping, beginUndoGrouping);
            REPL(original_endUndoGrouping, endUndoGrouping);
            REPL(original_dealloc, dealloc);
...
- (void)replacement_registerUndoWithTarget:(id)target selector:(SEL)selector object:(id)anObject;
{
    // Do this before logging so that the 'BEGIN' log happens first (probably in auto-group creation mode)
    original_registerUndoWithTargetSelectorObject(self, _cmd, target, selector, anObject);
...
- (void)replacement_forwardInvocation:(NSInvocation *)anInvocation;
{
    ...
    // Do this before logging so that the 'BEGIN' log happens first (probably in auto-group creation mode)
    original_forwardInvocation(self, _cmd, anInvocation);
...
- (void)replacement_beginUndoGrouping;
{
    ...
    original_beginUndoGrouping(self, _cmd);
}

- (void)replacement_endUndoGrouping;
{
    original_endUndoGrouping(self, _cmd);
    if ((OFUndoManagerLoggingOptions != OFUndoManagerNoLogging)) {
        OFUndoManagerLoggingState *state = _OFUndoManagerLoggingStateGet(self);
        state->indentLevel--;
