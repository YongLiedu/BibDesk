...
RCS_ID("$Header: svn+ssh://source.omnigroup.com/Source/svn/Omni/tags/OmniSourceRelease_2007-10-25/OmniGroup/Frameworks/OmniFoundation/OpenStepExtensions.subproj/NSData-OFExtensions.m 93315 2007-10-24 11:51:50Z bungi $")
...
- (NSData *)compressedBzip2Data;
{
    ...
    if (length) {
        ...
        if (err != BZ_OK) {
            ...
            NSString *reason = [NSString stringWithFormat:NSLocalizedStringFromTableInBundle(@"Unable to compress data (bzip2 rc:%d '%s')", @"OmniFoundation", [OFObject bundle], @"compression exception format"), err, BZ2_bzerror(bzFile, &err)];
...
- (NSData *)decompressedBzip2Data;
{
    ...
    do {
        ...
        if (err != BZ_OK && err != BZ_STREAM_END) {
            // Create exception before closing file since we read from the file
            NSString *reason = [NSString stringWithFormat:NSLocalizedStringFromTableInBundle(@"Unable to decompress data (bzip2 rc:%d '%s')", @"OmniFoundation", [OFObject bundle], @"decompression exception format"), err, BZ2_bzerror(bzFile, &err)];
            NSException *exc = [NSException exceptionWithName:NSInvalidArgumentException reason:reason userInfo:nil];
            fclose(dataFile);
