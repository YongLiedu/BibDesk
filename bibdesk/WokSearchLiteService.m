	...
	for( cur = cur->children ; cur != NULL ; cur = cur->next ) {
		if(cur->type == XML_ELEMENT_NODE) {
			...
			if(xmlStrEqual(cur->name, (const xmlChar *) "supportingWebServiceException")) {
				id newChild = [WokSearchLiteService_SupportingWebServiceException deserializeNode:cur];
	...
	for( cur = cur->children ; cur != NULL ; cur = cur->next ) {
		if(cur->type == XML_ELEMENT_NODE) {
			if(xmlStrEqual(cur->name, (const xmlChar *) "faultInformation")) {
				id newChild = [WokSearchLiteService_FaultInformation deserializeNode:cur];
				self.faultInformation = newChild;
			}
			if(xmlStrEqual(cur->name, (const xmlChar *) "rawFaultInformation")) {
				id newChild = [WokSearchLiteService_RawFaultInformation deserializeNode:cur];
